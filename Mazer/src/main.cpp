...
class MazerFPS : public olcConsoleGameEngine {
    ...
    float speed = 5.0f;
    float rotateMult = 0.4f;
    float strafeMult = 0.5f;
...
public:
    ...
    virtual bool OnUserUpdate(float timeDelta) override {
        // CCW Rotation
        if (m_keys[L'A'].bHeld)
            playerA -= (speed * rotateMult) * timeDelta;
        ...
        // Forward Movement
        if (m_keys[L'W'].bHeld) {
            playerX += std::sinf(playerA) * speed * timeDelta;
            playerY += std::cosf(playerA) * speed * timeDelta;
        ...
        // Strafe Left
        if (m_keys[L'Q'].bHeld) {
            playerX -= std::cosf(playerA) * (speed * strafeMult) * timeDelta;
            playerY += std::sinf(playerA) * (speed * strafeMult) * timeDelta;
            // Prevent walking out of bounds
            if (map[(int)playerY * mapWidth + (int)playerX] == '#') {
                playerX += std::cosf(playerA) * (speed * strafeMult) * timeDelta;
                playerY -= std::sinf(playerA) * (speed * strafeMult) * timeDelta;
